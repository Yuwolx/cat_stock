...
def _call_claude(prompt: str, max_tokens: int = 600) -> str | None:
...
def generate_stock_column(payload: dict) -> dict | None:
    """개별 종목 AI 분석 칼럼 생성"""
...
- 마지막 문장은 전망 또는 주목 포인트로 마무리"""
    ...
    if not text:
        return None
    return _split_title_body(text, fallback_title=f"{basics.get('name','?')} 분석")


def generate_market_column(payload: dict) -> dict | None:
    """시황 브리핑 AI 칼럼 생성"""
...
- 투자 권유 없이 시장 해석 관점"""
    ...
    if not text:
        return None
    return _split_title_body(text, fallback_title=f"{target_date} 시황")
